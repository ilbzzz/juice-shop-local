  ...
  void (async () => {
    ...
    try {
      ...
      if (req.body.layout) {
        ...
        const isForbiddenFile: boolean = (filePath.toLowerCase().includes('ftp') || filePath.toLowerCase().includes('ctf.key') || filePath.toLowerCase().includes('encryptionkeys')) || !filePath.toLowerCase().startsWith(viewsPath.toLowerCase() + path.sep)
        if (!isForbiddenFile) {
