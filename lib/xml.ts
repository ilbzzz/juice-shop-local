...
async function loadLibxml2 () {
  if (libxml2Promise == null) {
    libxml2Promise = (async () => {
      ...
      // Do not register fs input providers to ensure host filesystem access is blocked.
      return libxml2
    })()
  }
...
// Parses XML safely with external entity loading disabled to prevent XXE.
// The parse runs in a vm context with a timeout so entity-expansion bombs
// surface as a "Script execution timed out" error instead of hanging the process.
export async function parseXmlString (data: string, timeoutMs = 2000): Promise<string> {
  const libxml2 = await loadLibxml2()
  // Disable XML_PARSE_DTDLOAD and use XML_PARSE_NONET + XML_PARSE_NO_XXE.
  const option = libxml2.ParseOption.XML_PARSE_NOENT | libxml2.ParseOption.XML_PARSE_NONET | libxml2.ParseOption.XML_PARSE_NO_XXE | libxml2.ParseOption.XML_PARSE_NOBLANKS | libxml2.ParseOption.XML_PARSE_NOCDATA
