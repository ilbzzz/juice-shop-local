...
async function loadLibxml2 () {
  if (libxml2Promise == null) {
    libxml2Promise = (async () => {
      ...
      // Filesystem input providers are NOT registered to prevent XXE attacks.
      return libxml2
    })()
  }
...
// Parses XML with entity substitution enabled for internal entities only.
// External entity loading is disabled to prevent XXE attacks. The parse runs
...
export async function parseXmlString (data: string, timeoutMs = 2000): Promise<string> {
  ...
  // XML_PARSE_NOENT: Substitute entities (internal entities only as external entity loading is disabled)
  // XML_PARSE_NONET: Forbid network access
  // XML_PARSE_NO_XXE: Disable loading of external entities
  const option = libxml2.ParseOption.XML_PARSE_NOENT |
    libxml2.ParseOption.XML_PARSE_NOBLANKS |
    libxml2.ParseOption.XML_PARSE_NOCDATA |
    libxml2.ParseOption.XML_PARSE_NONET |
    (libxml2.ParseOption.XML_PARSE_NO_XXE ?? 0)
