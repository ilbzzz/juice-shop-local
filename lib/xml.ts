...
export async function parseXmlString (data: string, timeoutMs = 2000): Promise<string> {
  ...
  const option = (libxml2.ParseOption.XML_PARSE_NOENT ?? 0) | (libxml2.ParseOption.XML_PARSE_NONET ?? 0) | (libxml2.ParseOption.XML_PARSE_NOBLANKS ?? 0) | (libxml2.ParseOption.XML_PARSE_NOCDATA ?? 0)
