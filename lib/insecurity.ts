...
const initKeys = () => {
  const envKey = process.env.JWT_PRIVATE_KEY ?? process.env.PRIVATE_KEY
  let privKey = envKey ? (envKey.includes('\\n') ? envKey.replace(/\\n/g, '\n') : envKey) : null
  if (!privKey && fs?.existsSync('encryptionkeys/jwt.key')) {
    privKey = fs.readFileSync('encryptionkeys/jwt.key', 'utf8')
  }

  let pubKey = ''
  if (privKey) {
    try {
      pubKey = crypto.createPublicKey(privKey).export({ type: 'pkcs1', format: 'pem' }).toString()
    } catch {
      privKey = null
    }
  }
  ...
  const envPubKey = process.env.JWT_PUBLIC_KEY ?? process.env.PUBLIC_KEY
  if (envPubKey) {
    pubKey = envPubKey.includes('\\n') ? envPubKey.replace(/\\n/g, '\n') : envPubKey
  }
