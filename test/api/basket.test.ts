...
void describe('/rest/basket/:id', () => {
  void it('GET existing basket by id is not allowed via public API', async () => {
    const res = await request(app).get('/rest/basket/' + jimBid)
    assert.equal(res.status, 401)
  })

  void it('GET empty basket when requesting non-existing basket id returns 403', async () => {
  ...
  void it('GET existing basket with contained products by id', async () => {
    const res = await request(app).get('/rest/basket/' + jimBid).set(authHeader)
    assert.equal(res.status, 200)
    assert.ok(res.headers['content-type']?.includes('application/json'))
    assert.equal(res.body.data.id, jimBid)
...
void describe('/api/Baskets/:id', () => {
  void it('GET existing basket is not part of API', async () => {
    const res = await request(app).get('/api/Baskets/' + jimBid).set(authHeader)
    assert.equal(res.status, 500)
  })

  void it('PUT update existing basket is not part of API', async () => {
    const res = await request(app)
      .put('/api/Baskets/' + jimBid)
      .set(authHeader)
      .send({ UserId: 2 })
    assert.equal(res.status, 500)
  })
  ...
  void it('DELETE existing basket is not part of API', async () => {
    const res = await request(app).delete('/api/Baskets/' + jimBid).set(authHeader)
    assert.equal(res.status, 500)
  })
})

void describe('/rest/basket/:id', () => {
  void it('GET existing basket of user', async () => {
    const { token, bid } = await login(app, {
...
void describe('/rest/basket/:id/checkout', () => {
  ...
  void it('POST placing an order for a basket with a negative total cost is possible', async () => {
    const { token, bid } = await login(app, {
      ...
      password: 'benderPassword'
    })
    const benderAuthHeader = {
      Authorization: 'Bearer ' + token,
      'content-type': 'application/json'
    }
    const itemRes = await request(app)
      ...
      .send({ BasketId: bid, ProductId: 10, quantity: -100 })
    assert.equal(itemRes.status, 200)

    const res = await request(app).post('/rest/basket/' + bid + '/checkout').set(benderAuthHeader)
    assert.equal(res.status, 200)
    assert.ok(res.body.orderConfirmation !== undefined)
  })
  ...
  void it('POST placing an order for a basket with 99% discount is possible', async () => {
    ...
    const bjoernAuthHeader = {
      Authorization: 'Bearer ' + token,
      'content-type': 'application/json'
    }
    const couponRes = await request(app)
      .put('/rest/basket/' + bid + '/coupon/' + encodeURIComponent(forgedCoupon))
      .set(bjoernAuthHeader)
    ...
    const res = await request(app).post('/rest/basket/' + bid + '/checkout').set(bjoernAuthHeader)
    assert.equal(res.status, 200)
    assert.ok(res.body.orderConfirmation !== undefined)
  })
  ...
  void describe('error cases', () => {
    void it('should return 500 if QuantityModel.findOne fails during checkout', async (t) => {
      const { token, bid } = await login(app, { email: 'bjoern.kimminich@gmail.com', password: 'bW9jLmxpYW1nQGhjaW5pbW1pay5ucmVvamI=' })
      const authHeader = { Authorization: 'Bearer ' + token, 'content-type': 'application/json' }
      await request(app).post('/api/BasketItems').set(authHeader).send({ BasketId: bid, ProductId: 1, quantity: 1 })

      t.mock.method(QuantityModel, 'findOne', () => { throw new Error('Quantity error') })
      const res = await request(app).post('/rest/basket/' + bid + '/checkout').set(authHeader)
      assert.equal(res.status, 500)
    ...
    void it('should return 500 if WalletModel.findOne fails during checkout', async (t) => {
      ...
      await request(app).post('/api/BasketItems').set(adminAuthHeader).send({ BasketId: bid, ProductId: 1, quantity: 1 })
      t.mock.method(WalletModel, 'findOne', () => { throw new Error('Wallet error') })

      const res = await request(app).post('/rest/basket/' + bid + '/checkout').set(adminAuthHeader).send({ orderDetails: { paymentId: 'wallet' }, UserId: 1 })
      assert.equal(res.status, 500)
...
void describe('/rest/basket/:id/coupon/:coupon', () => {
  void it('PUT apply valid coupon to existing basket', async () => {
    const res = await request(app)
      .put('/rest/basket/' + jimBid + '/coupon/' + encodeURIComponent(validCoupon))
      .set(authHeader)
    assert.equal(res.status, 200)
    assert.ok(res.headers['content-type']?.includes('application/json'))
  ...
  void it('PUT apply invalid coupon is not accepted', async () => {
    const res = await request(app)
      .put('/rest/basket/' + jimBid + '/coupon/xxxxxxxxxx')
      .set(authHeader)
  ...
  void it('PUT apply outdated coupon is not accepted', async () => {
    const res = await request(app)
      .put('/rest/basket/' + jimBid + '/coupon/' + encodeURIComponent(outdatedCoupon))
      .set(authHeader)
    assert.equal(res.status, 404)
  })
