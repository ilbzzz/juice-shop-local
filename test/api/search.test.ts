...
void describe('/rest/products/search', () => {
  ...
  void it('GET product search handles quotes safely without SQL Injection vulnerability', async () => {
    const res = await request(app)
  ...
  void it('GET product search handles malformed SQL injection payload safely', async () => {
    const res = await request(app)
  ...
  void it('GET product search handles single parenthesis SQL injection payload safely', async () => {
    const res = await request(app)
      .get("/rest/products/search?q=') union select id,email,password from users--")
    assert.equal(res.status, 200)
    assert.ok(res.headers['content-type']?.includes('application/json'))
  ...
  void it('GET product search handles SELECT * UNION attack safely', async () => {
    const res = await request(app)
  ...
  void it('GET product search prevents UNION SELECT attacks with Users table and fixed columns', async () => {
    const res = await request(app)
  ...
  void it('GET product search prevents UNION SELECT attacks with Users table and required columns', async () => {
    const res = await request(app)
  ...
  void it('GET product search prevents UNION SELECT attacks with sqlite_master table', async () => {
    const res = await request(app)
  ...
  void it('GET product search cannot select logically deleted christmas special via SQL comment injection', async () => {
    const res = await request(app)
  ...
  void it('GET product search cannot select logically deleted unsafe product via SQL comment injection', async () => {
    const res = await request(app)
