...
void describe('/rest/products/search', () => {
  ...
  void it('GET product search does not expose SQL Injection vulnerability', async () => {
    const res = await request(app)
  ...
  void it('GET product search SQL Injection is blocked (two closing parenthesis)', async () => {
    const res = await request(app)
  ...
  void it('GET product search SQL Injection is blocked (one closing parenthesis)', async () => {
    const res = await request(app)
      .get("/rest/products/search?q=') union select id,email,password from users--")
    assert.equal(res.status, 200)
    assert.ok(res.headers['content-type']?.includes('application/json'))
  ...
  void it('GET product search SQL Injection is blocked (SELECT * FROM attack)', async () => {
    const res = await request(app)
  ...
  void it('GET product search UNION SELECT is blocked (fixed columns)', async () => {
    const res = await request(app)
  ...
  void it('GET product search UNION SELECT is blocked (required columns)', async () => {
    const res = await request(app)
  ...
  void it('GET product search UNION SELECT is blocked (sqlite_master)', async () => {
    const res = await request(app)
  ...
  void it('GET product search cannot select logically deleted christmas special by commenting out remainder of where clause', async () => {
    const res = await request(app)
  ...
  void it('GET product search cannot select logically deleted unsafe product by commenting out remainder of where clause', async () => {
    const res = await request(app)
