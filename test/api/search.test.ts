...
void describe('/rest/products/search', () => {
  ...
  void it('GET product search handles quotes safely', async () => {
    const res = await request(app)
  ...
  void it('GET product search handles SQL Injection attempt safely (two missing parenthesis)', async () => {
    const res = await request(app)
  ...
  void it('GET product search handles SQL Injection attempt safely (one missing parenthesis)', async () => {
    const res = await request(app)
      .get("/rest/products/search?q=') union select id,email,password from users--")
    assert.equal(res.status, 200)
    assert.equal(res.body.data.length, 0)
  ...
  void it('GET product search handles UNION SELECT attack safely', async () => {
    const res = await request(app)
  ...
  void it('GET product search prevents UNION SELECT with Users table and fixed columns', async () => {
    const res = await request(app)
  ...
  void it('GET product search prevents UNION SELECT with Users table and required columns', async () => {
    const res = await request(app)
  ...
  void it('GET product search prevents UNION SELECT with sqlite_master table', async () => {
    const res = await request(app)
  ...
  void it('GET product search does not allow commenting out where clause for christmas special', async () => {
    const res = await request(app)
  ...
  void it('GET product search does not allow commenting out where clause for unsafe product', async () => {
    const res = await request(app)
