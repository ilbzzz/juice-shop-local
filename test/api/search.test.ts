...
void describe('/rest/products/search', () => {
  ...
  void it('GET product search with SQL Injection attempt returns no products', async () => {
    const res = await request(app)
  ...
  void it('GET product search SQL Injection attempt with union returns no products', async () => {
    const res = await request(app)
  ...
  void it('GET product search SQL Injection attempt with one missing closing parenthesis returns no products', async () => {
    const res = await request(app)
      .get("/rest/products/search?q=') union select id,email,password from users--")
    assert.equal(res.status, 200)
    assert.ok(res.headers['content-type']?.includes('application/json'))
  ...
  void it('GET product search SQL Injection attempt for SELECT * FROM attack returns no products', async () => {
    const res = await request(app)
